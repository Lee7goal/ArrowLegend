export default class DateUtils {
    ...
    //3: 00:00
    public static getFormatBySecond3(t:number = 0):string {
        var hourst:number = Math.floor(t / 3600);
        var minst:number = Math.floor((t - hourst * 3600) / 60);
        var secondt:number = Math.floor((t - hourst * 3600) % 60);
        var mins:string;
    ...
    //2:yyyy-mm-dd h:m:s
    public  static getFormatBySecond2(time:number):string {
    ...
    //4:xx天前，xx小时前，xx分钟前
    public  static getFormatBySecond4(time:number):string {
    ...
    public  static getFormatBySecond5(time:number):string {
        //每个时间单位所对应的秒数
